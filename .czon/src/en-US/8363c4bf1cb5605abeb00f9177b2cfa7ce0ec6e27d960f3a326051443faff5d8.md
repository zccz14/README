...
"summary": "This paper proposes Full-Spectrum Analysis (FSA), an investment and trading strategy framework optimized based on the Kelly criterion. The article first analyzes the limitations of the traditional Kelly formula in investment applications, such as the lack of consideration for leverage and short selling, and the issue of liquidation timing. Then, FSA constructs a systematic trading decision model by defining the outcome space, calculating optimal leverage, and compound yield. The paper elaborates on the mathematical principles of FSA, including the calculation of expected yield and compound yield, as well as the algorithm for solving optimal leverage using Newton's method. Additionally, it introduces historical backtesting methods (such as Gross Profit Margin, GPM calculation), considerations for live trading modules, and measures to handle black swan events. The core advantage of FSA lies in its ability to utilize imperfect probability information to maximize long-term returns by optimizing leverage decisions, thereby reducing the high requirement for information quality."
"tags":
...
1.  It does not utilize borrowing leverage or short selling, confining its conclusions to the 0~1 range of actual leverage.
2.  It assumes the gambler bets on a specific event symbol, receiving a payout if it occurs next, otherwise losing the entire bet. But traders can only bet on long/short positions, and the corresponding bullish/bearish events yield different results and returns across different time windows.
3.  **Liquidation Timing:** Kelly assumes the end of the gamble and liquidation are not determined by the gambler's will. However, traders can decide to exit or continue holding at any time.
4.  **Redundant Bayesian Methods:** It assumes event development is related to prior and conditional probabilities. Removing Bayesian methods does not affect the derived mathematical essence. It increases complexity, making direct application of the paper's formulas difficult; practical use requires simplification.
...
2.  **Deterministic Probability:** Let the probability of outcome $i \in X$ occurring be $P_i$, ($0 \le P_i \le 1$).
    1.  ✅ The probability of each outcome must be assessed deterministically.
    2.  ❌ Stating that the possibility of a certain outcome is "over 80%": 80%～ 100%, violates deterministic probability.
3.  **Mutual Exclusivity:** It is impossible for two outcomes to occur simultaneously.
    ...
    2.  ❌ If outcome events have intersections, e.g., "hold for 20 minutes" and "drop 20%" events might occur simultaneously, violating mutual exclusivity.
4.  **Completeness:** The sum of probabilities for all outcomes is 100%. There are no outcomes outside the predefined set, i.e., $\sum_{i \in X} P_i = 1$.
    ...
    2.  ❌ If the outcome events only define "rise over 20%" and "drop over 20%", without defining "fluctuation between -20% and 20%", it violates completeness.
...
The concept of expected yield is intuitive and convenient for calculation. The problems with expected yield are:

1.  It leads to extreme decisions—either zero position or full position—unable to control bankruptcy risk. If the expected yield is positive, it tends to infinitely amplify leverage to seek higher returns, even if high leverage can lead to ruin.
...
Recap the problem: Given outcome space $X$, probability distribution $P_i$, yield $E_i$, feasible region $[L, R]$, find the leverage rate $k \in [L, R]$ that maximizes the yield:
...
If there are $N$ distinct values of $R_i$, the highest degree of the above polynomial equation is $N-1$.
...
For trading scenarios, there might be countless outcomes with different yields, so we can only seek numerical solutions.
...
#### The Case Where Newton's Method Exceeds the Feasible Region
...
Use a minimal example to illustrate this situation:
...
but when using Newton's method, the first iteration point is
...
2.  Based on return rates $r_i$, calculate the basic feasible region $K$
3.  Clip the feasible region: $L \gets \max(L, \min K), R \gets \min(R, \max K)$
4.  Loop for a maximum of $N$ times:
